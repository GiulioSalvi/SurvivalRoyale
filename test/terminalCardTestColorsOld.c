...
void drawCard(int startingX, int startingY, int suit, int rank) {
    ...
    printf("%s", color); // Set color for the border
    ...
    printf("%s", RESET); // Reset color after drawing the borders
    ...
    printf("%s%s\033[0m", color, cardSuitString);

    // Print card rank
    moveCursorAt(startingX + CARD_HEIGHT - 2, centerCol - strlen(cardRankString) / 2);
    printf("%s%s\033[0m", color, cardRankString);
}
